...
void Dictionary::add(const std::string& w) {
  ...
  if (w.at(0) == '[' || w.at(0) == '#' || w.at(0) == '<') {
    special_tokens_.insert(w);
  }
}
...
bool Dictionary::isSpecialToken(const std::string& w) const {
  /*for (auto it = special_tokens_.cbegin(); it != special_tokens_.cend(); ++it) {
    if (*it == w) {
      return true;
    }
  }
  return false;*/
  if (special_tokens_.find(w) == special_tokens_.end()) {
    return false;
  } else {
    return true;
  }
...
bool Dictionary::isSpecialToken(const int32_t& wid) const {
  std::string w = getWord(wid);
  return isSpecialToken(w);
}
...
void Dictionary::computeSubwords(
    ...
    std::vector<int32_t>& ngrams,
    std::vector<std::string>* substrings) const {
  if (isSpecialToken(word.substr(1, word.size()-2))) {
...
void Dictionary::readFromFile(std::istream& in) {
  ...
  assert(special_tokens_.erase(EOS));
...
bool Dictionary::readConfusion(std::istream& in, std::vector<std::string>& confusion) const {
  std::string word;
  bool wordFlag = true;
  confusion.clear();

  // Each confusion starts with "[" word.
  if (readWord(in, word)) {
    if (word == EOS) {
      confusion.push_back(word);
      return true;
    } else {
      assert(word == "[");
    }
  ...
  while (readWord(in, word)) {
    if (wordFlag) {
      // Each confusion ends with "]" word.
      if (word == "]") {
        if (args_->model == model_name::c2v_t) {
          confusion.resize(1);
        }
...
int32_t Dictionary::getSausage(
  ...
  //assert(readWord(in, word)); // First word is utterence id

  while (readConfusion(in, confusion)) {
    c.clear();
    for (auto it = confusion.cbegin(); it != confusion.cend(); ++it) {
      int32_t h = find(*it);
      int32_t wid = word2int_[h];
      if (wid < 0) {
        continue;
    ...
    //std::cerr << "[ ";
    //std::copy(confusion.begin(), confusion.end(), std::ostream_iterator<std::string>(std::cerr, " "));
    //std::cerr << " ] ";
  }
  //std::cerr << std::endl;
  return ntokens;
...
void Dictionary::readSausageFromFile(std::istream& in) {
  int64_t minThreshold = 1;
  std::string word;
  std::vector<std::string> confusion;
  int32_t nconfusions_ = 0;
  while (readConfusion(in, confusion)) {
    for (auto it = confusion.cbegin(); it != confusion.cend(); ++it) {
      add(*it);
      if (ntokens_ % 1000000 == 0 && args_->verbose > 1) {
  ...
  if (args_->verbose > 0) {
    std::cerr << "\rRead " << ntokens_ / 1000000 << "M words" << std::endl;
    std::cerr << "Number of confusions:  " << nconfusions_ << std::endl;
    std::cerr << "Number of words:  " << nwords_ << std::endl;
    std::cerr << "Number of labels: " << nlabels_ << std::endl;
    std::cerr << "Special tokens: ";
    std::copy(special_tokens_.cbegin(), special_tokens_.cend(), std::ostream_iterator<std::string>(std::cerr, " "));
    std::cerr << std::endl;
  }
  if (size_ == 0) {
    throw std::invalid_argument(
...
void Dictionary::addDict(std::shared_ptr<Dictionary>& dict, bool reset) {
  ...
  std::cerr << " num_tokens=" << ntokens_ << " size=" << size_ << " words=" << nwords_<< " labels=" << nlabels_ << std::endl;

  if (args_->verbose > 0) {
    std::cerr << "\rRead " << dict->ntokens() / 1000000 << "M words" << std::endl;
    std::cerr << "Number of words:  " << nwords_ << std::endl;
    std::cerr << "Number of labels: " << nlabels_ << std::endl;
  }
  if (size_ == 0) {
