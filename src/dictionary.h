...
class Dictionary {
 ...
 public:
  ...
  bool readConfusion(std::istream& in, std::vector<std::string>& confusion) const;
  void readFromFile(std::istream&);
  void readSausageFromFile(std::istream& in);
  ...
  int32_t getLine(std::istream&, std::vector<int32_t>&, std::vector<int32_t>&)
      const;
  int32_t getSausage(std::istream& in, std::vector<std::vector<int32_t>>& sausage, std::minstd_rand& rng) const;
  ...
  void addDict(std::shared_ptr<Dictionary>& dict, bool reset);
  bool isSpecialToken(const int32_t& wid) const;
  bool isSpecialToken(const std::string& w) const;
  void init();
};
