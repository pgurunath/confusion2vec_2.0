...
void FastText::loadModel(std::istream& in) {
  ...
  if (!quant_input && dict_->isPruned()) {
    throw std::invalid_argument(
        "Invalid model file.\n"
        "Please download the updated model from www.c2v_fasttext.cc.\n"
...
void FastText::interconfusion(
  ...
  for (int32_t cidx = 0; cidx < sausage.size(); cidx++) {
    int32_t boundary = uniform(state.rng);
    for (int32_t widx = 0; widx < sausage[cidx].size(); widx++) {
//      if (dict_->isSpecialToken(sausage[cidx][widx]))
//        continue;
      const std::vector<int32_t>& ngrams = dict_->getSubwords(sausage[cidx][widx]);
      for (int32_t cc = -boundary; cc <= boundary; cc++) {
        if (cc != 0 && cidx + cc >= 0 && cidx + cc < sausage.size()) {
          for (int32_t c = 0; c < sausage[cc+cidx].size(); c++) {
            //std::cerr << dict_->getWord(sausage[cidx][widx]) << " -> " << dict_->getWord(sausage[cc+cidx][c]) << std::endl;
//            if (dict_->isSpecialToken(sausage[cc+cidx][c]))
//              continue;
            model_->update(ngrams, sausage[cc+cidx], c, lr, state);
          }
        }
      }
    }
...
void FastText::intraconfusion(
  ...
  for (int32_t cidx = 0; cidx < sausage.size(); cidx++) {
    for (int32_t widx = 0; widx < sausage[cidx].size(); widx++) {
      ...
      const std::vector<int32_t>& ngrams = dict_->getSubwords(sausage[cidx][widx]);
      for (int32_t cc = -boundary; cc <= boundary; cc++) {
        if (cc != 0 && widx + cc >= 0 && widx + cc < sausage[cidx].size()) {
          model_->update(ngrams, sausage[cidx], widx + cc, lr, state);
        }
      }
    }
  }
...
void FastText::hybridconfusion(
    Model::State& state,
    real lr,
    const std::vector<std::vector<int32_t>>& sausage) {
  intraconfusion(state, lr, sausage);
  interconfusion(state, lr, sausage);
}
...
void FastText::getSentenceVector(std::istream& in, c2v_fasttext::Vector& svec) {
  svec.zero();
  if (args_->model == model_name::sup) {
...
void FastText::trainThread(int32_t threadId, const TrainCallback& callback) {
  ...
  if (modelToString(args_->model).find("c2v_") != std::string::npos && threadId != 0) {
    std::minstd_rand tmp(0);
    assert(dict_->getLine(ifs, line, tmp));
  } 
  ...
  std::vector<std::vector<int32_t>> sausage;
  ...
  try {
    while (keepTraining(ntokens)) {
      ...
      if (args_->model == model_name::sup) {
        ...
        localTokenCount += dict_->getLine(ifs, line, state.rng);
        skipgram(state, lr, line);
      } else if (args_->model == model_name::c2v_t) {
        localTokenCount += dict_->getSausage(ifs, sausage, state.rng);
        topconfusion(state, lr, sausage);
      } else if (args_->model == model_name::c2v_a) {
        localTokenCount += dict_->getSausage(ifs, sausage, state.rng);
        intraconfusion(state, lr, sausage);
      } else if (args_->model == model_name::c2v_c) {
        localTokenCount += dict_->getSausage(ifs, sausage, state.rng);
        interconfusion(state, lr, sausage);
      } else if (args_->model == model_name::c2v_h) {
        localTokenCount += dict_->getSausage(ifs, sausage, state.rng);
        hybridconfusion(state, lr, sausage);
      }
    ...
    std::cerr << "NaNs encountered" << std::endl;
...
void FastText::train(const Args& args, const TrainCallback& callback) {
  ...
  std::cerr << "-----------------Training args-------------------" << std::endl;
  args_->dump(std::cerr);
  ...
  if (!args.incr) {
    ...
    if (args_->model == model_name::c2v_t || args_->model == model_name::c2v_a || args_->model == model_name::c2v_c || args_->model == model_name::c2v_h) {
      dict_->readSausageFromFile(ifs);
    } else {
    ...
    std::cerr << "-----------------Provided args-------------------" << std::endl;
    args_->dump(std::cerr);
    std::cerr << "-----------------inputModel args-------------------" << std::endl;
    old_args_->dump(std::cerr);
    

    if (old_args_->model != model_name::sg && modelToString(args_->model).find("c2v_") != std::string::npos) {
      std::cerr << "Mismatch models. inputModel type (" << modelToString(old_args_->model) << ") vs. specified argument (" << modelToString(args_->model) << ")" << std::endl;
      exit(EXIT_FAILURE);
    } else if (args_->model != old_args_->model && modelToString(args_->model).find("c2v_") == std::string::npos) {
      std::cerr << "Mismatch models. inputModel type (" << modelToString(old_args_->model) << ") vs. specified argument (" << modelToString(args_->model) << ")" << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    std::shared_ptr<Dictionary> dictInModel = std::make_shared<Dictionary>(args_, inModel);
    ...
    std::shared_ptr<Dictionary> dictInData = std::make_shared<Dictionary>(args_);//, ifs);
    if (args_->model == model_name::c2v_t || args_->model == model_name::c2v_a || args_->model == model_name::c2v_c || args_->model == model_name::c2v_h) {
      dictInData->readSausageFromFile(ifs);
      //dict_->readSausageFromFile(ifs);
    } else {
    ...
    std::shared_ptr<DenseMatrix> inputInModel = std::make_shared<DenseMatrix>();
    std::shared_ptr<QuantMatrix> qinputInModel = std::make_shared<QuantMatrix>();
    ...
    std::cerr << "Loaded Input matrix dimension = " << inputInModel->size(0) << " x " << inputInModel->size(1) << std::endl;
    ...
    if (quant_) {
      ...
      for (auto i = 0; i < dictInModel->nwords(); ++i) {
        ...
        if (0 <= idx && idx < dict_->nwords()) {
          ...
          input_->addVectorToRow(wordVector, idx, 1.0);
          wordVector.zero();
        } else {
    ...
    std::shared_ptr<DenseMatrix> outputInModel = std::make_shared<DenseMatrix>();
    std::shared_ptr<QuantMatrix> qoutputInModel = std::make_shared<QuantMatrix>();
    ...
    std::cerr << "Loaded Output matrix dimension = " << outputInModel->size(0) << " x " << outputInModel->size(1) << std::endl;
      ...
      if (args_->model == model_name::sup) {
        ...
        for (auto i = 0; i < dictInModel->nlabels(); ++i) {
          ...
          if (0 <= idx && idx < dict_->nwords() + dict_->nlabels()) {
            ...
            output_->addVectorToRow(labelVector, idx - dict_->nwords(), 1.0);
            labelVector.zero();
          }
        }
      } else {
        ...
        for (auto i = 0; i < dictInModel->nwords(); ++i) {
          ...
          if (0 <= idx && idx < dict_->nwords()) {
            ...
            output_->addVectorToRow(wordVector, idx, 1.0);
            wordVector.zero();
          }
        }
      }
  ...
  std::cerr << "Dictionary: nwords=" << dict_->nwords() << " nlabels=" << dict_->nlabels() << " ntokens=" << dict_->ntokens() << std::endl;
...
std::string FastText::boolToString(bool b) const {
  if (b) {
    return "true";
  } else {
    return "false";
...
std::string FastText::modelToString(model_name mn) const {
  switch (mn) {
    ...
    case model_name::sup:
      return "sup";
    case model_name::c2v_t:
      return "c2v_t";
    case model_name::c2v_a:
      return "c2v_a";
    case model_name::c2v_c:
      return "c2v_c";
    case model_name::c2v_h:
      return "c2v_h";
  }
